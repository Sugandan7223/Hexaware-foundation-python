...
class Database:
    def __init__(self):
        self.conn = PetsPals.dao.connect_to_sql_server()
    ...
    def register_participant(self, participant_id, participant_name, participant_email, event_id, city):
        try:
            ...
            raise PetsPals.exception.DatabaseOperationException("Failed to register participant.")
...
class CashDonation(PetsPals.entity.Donation):
...
class ItemDonation(PetsPals.entity.Donation):
    def record_donation(self):
        try:
            cursor = self.conn.cursor()
            cursor.execute(
